...
(function ( angular ) {
	...
	angular.module( 'angularTreeview', ['ui.router'] ).directive( 'treeModel', function( $compile,$location,$rootScope,$state) {
		return {
			restrict: 'A',
				...
				if( treeModel && treeModel.length ) {
					...
					if( attrs.angularTreeview ) {
						...
						scope.selectNodeLabel = scope.selectNodeLabel || function( selectedNode ){
							...
							$rootScope.uishow = true;
							if(selectedNode.nodeUrl)
							{	
								$location.path($location.path() + '/' + selectedNode.nodeUrl);
							}
							
							else if(selectedNode.nodeState)
							{
								//$location.path($location.path() + '/' + 'home.html');
								$state.go(selectedNode.nodeState);
							}
